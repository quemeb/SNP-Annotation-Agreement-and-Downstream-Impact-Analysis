...
def load_data(file_path):
    columns_needed = ["chr", "pos", "ref", "alt", "rs_dbSNP151", "ANNOVAR_ensembl_Gene_ID",
                      "ANNOVAR_ensembl_Closest_gene(intergenic_only)", "SnpEff_ensembl_Gene_ID", "VEP_ensembl_Gene_ID"]
...
def load_gene_mapping_ensembl_to_hgnc(file_path):
    """
    Load gene mapping data and create a dictionary to map:
    - Ensembl Gene ID → HGNC ID
    ...
    -------
    dict
        Mapping from Ensembl Gene ID to HGNC ID.
    """
    df = pd.read_csv(file_path, dtype=str).fillna("")

    # Build Ensembl-to-HGNC mapping
    ensembl_mapping = {
        row["ensembl_gene_id"].strip(): row["uniprot_ids"].strip()
        for _, row in df.iterrows() if row["ensembl_gene_id"].strip()
    }

    return ensembl_mapping

def map_ensembl_to_hgnc_genes(ensembl_genes_list, ensembl_mapping):
    """
    Map Ensembl gene IDs to HGNC IDs using the provided mapping.

    Parameters
    ----------
    ensembl_genes_list : list of lists
        Each sublist contains Ensembl gene IDs for a given SNP.
    ensembl_mapping : dict
        Mapping from Ensembl Gene ID → HGNC ID.

    Returns
    -------
    tuple:
        - list of lists: Mapped HGNC IDs per SNP (only successful matches).
    ...
    """
    mapped_genes_list = []          # Final list of mapped HGNC IDs per SNP
    total_unmatched_count = 0       # Tracks how many unmatched entries exist per SNP
    unmatched_genes_set = set()     # Tracks unique unmatched Ensembl IDs across the file

    for gene_list in ensembl_genes_list:
        matched = []
        seen_unmatched = set()  # To avoid counting the same unmatched gene multiple times per SNP

        for gene in gene_list:
            mapped_gene = ensembl_mapping.get(gene)
...
def data_process(file_path, ensembl_map):
    """
    ...
    AN_ID_genic = cdata["ANNOVAR_ensembl_Gene_ID"]  # Gene IDs for genic regions
    AN_ID_intergenic = cdata["ANNOVAR_ensembl_Closest_gene(intergenic_only)"]  # Closest gene for intergenic regions
    AN_ID_tog = [AN_ID_intergenic[i] if gene_id.startswith('.') else gene_id for i, gene_id in enumerate(AN_ID_genic)]

    SN_ID_tog = cdata["SnpEff_ensembl_Gene_ID"]  # Gene IDs for SnpEff
    ...
    VP_ID_tog = cdata["VEP_ensembl_Gene_ID"]  # Gene IDs for VEP
    ...
    AN_ID_genic, an_genic_unmatch, an_genic_unique = map_ensembl_to_hgnc_genes(extract(AN_ID_genic), ensembl_map)
    AN_ID_intergenic, an_inter_unmatch, an_inter_unique = map_ensembl_to_hgnc_genes(extract(AN_ID_intergenic), ensembl_map)

    SN_ID_genic, sn_genic_unmatch, sn_genic_unique = map_ensembl_to_hgnc_genes(extract(SN_ID_genic), ensembl_map)
    SN_ID_intergenic, sn_inter_unmatch, sn_inter_unique = map_ensembl_to_hgnc_genes(extract(SN_ID_intergenic), ensembl_map)

    VP_ID_genic, vp_genic_unmatch, vp_genic_unique = map_ensembl_to_hgnc_genes(extract(VP_ID_genic), ensembl_map)
    VP_ID_intergenic, vp_inter_unmatch, vp_inter_unique = map_ensembl_to_hgnc_genes(extract(VP_ID_intergenic), ensembl_map)
...
def process_all_files(input_path, output_path):
    ...
    ensembl_map = load_gene_mapping_ensembl_to_hgnc("[HGNC Mapping CSV file path]")

    for file_path in glob.glob(input_path):
        print(f"Processing file: {file_path}")
        results = data_process(file_path, ensembl_map)
    ...
    # Save to CSV
    genic_concat_df.to_csv(f"{output_path}/genic_concat_ensembl.csv", index=False)
    genic_concat_rate_df.to_csv(f"{output_path}/genic_concat_rate_ensembl.csv", index=False)
    genic_sunburst_df.to_csv(f"{output_path}/genic_sunburst_ensembl.csv", index=False)

    inter_concat_df.to_csv(f"{output_path}/inter_concat_ensembl.csv", index=False)
    inter_concat_rate_df.to_csv(f"{output_path}/inter_concat_rate_ensembl.csv", index=False)
    inter_sunburst_df.to_csv(f"{output_path}/inter_sunburst_ensembl.csv", index=False)
    
    # Save unmatched info
    genic_unmatched_df.to_csv(f"{output_path}/genic_unmatched_counts_ensembl.csv", index=False)
    inter_unmatched_df.to_csv(f"{output_path}/inter_unmatched_counts_ensembl.csv", index=False)


def main():
    # Define input and output paths
    input_path = "C:\\Users\\bryan\\OneDrive - University of Southern California\\Research\\Huaiyu Mi\\AnnoQ\\AnnoQ_data_all\\*.gz"
    output_path = "C:\\Users\\bryan\\OneDrive - University of Southern California\\Research\\Huaiyu Mi\\AnnoQ\\Results_ensembl_nested_NEW"
